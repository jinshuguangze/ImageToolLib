...
%		   并自动申请新的合适的内存，也提供便利的字段与函数以供手动检测对象内存是否足够
                ...
                case 1%当输入参数只为一个时，只允许出现正数  
                    if isscalar(varargin{1}) && isnumeric(varargin{1}) && varargin{1}>0        
                        ...
                        if varargin{1}>intmax('uint16')%输入参数过大
                            dynObj=zeros(1,intmax('uint16'));%默认输出
                            disp(['参数1：列数过大，最大不能超过',num2str(intmax('uint16')),'！']);             
                            return;
                        else%输入参数在范围内
                            dynObj=zeros(1,uint16(varargin{1}));
                        end
                    else%如果输入的数字不为正数
                        dynObj=0;%默认输出
                        disp('参数1：列数必须是一个正数！');
                    ...
                    if isscalar(varargin{1}) && isnumeric(varargin{1}) && varargin{1}>0
                        if isscalar(varargin{2}) && isnumeric(varargin{2}) && varargin{2}>0%数字加数字
                            ...
                            if varargin{1}*varargin{2}>intmax('uint16')%输入参数过大
                                dynObj=zeros(intmax('uint8'),intmax('uint8'));%默认输出
                                disp(['参数1：行数与参数2：列数的乘积过大，最大不能超过',...
                                    num2str(intmax('uint16')),'！']);
                                return;
                            else%输入参数在范围内                 
                                dynObj=zeros(uint16(varargin{1}),uint16(varargin{2}));
                            end
                            ...
                            switch varargin{2}
                                ...
                                case 'struct'%此时动态内存对象被赋予为结构体行向量，最长长度为intmax('uint64')
                                    if varargin{1}>intmax('uint64')%输入参数过大
                                        dynObj(1,intmax('uint64'))=struct;%默认输出
                                        disp(['参数1：列数过大，最大不能超过',num2str(intmax('uint64')),'！']);
                                        return;
                                    else%输入参数在范围内
                                        dynObj(1,uint64(varargin{1}))=struct;
                                    end

                                case 'cell'%此时动态内存对象被赋予为细胞行向量，且最长长度为intmax('uint16')
                                    if varargin{1}>intmax('uint16')%输入参数过大
                                        dynObj=cell(1,intmax('uint16'));%默认输出
                                        dynObj(:)={0};
                                        disp(['参数1：列数过大，最大不能超过',num2str(intmax('uint16')),'！']);
                                        return;
                                    else%输入参数在范围内
                                        dynObj=cell(1,varargin{1});
                                        dynObj(:)={0};
                                    end
                        ...
                        switch varargin{3}
                            ...
                            case 'struct'%此时动态内存对象被赋予为结构体数组，行数与列数都不超过intmax('uint64')
                                if varargin{1}>intmax('uint64')%参数1过大
                                    dynObj(intmax('uint64'),intmax('uint64'))=struct;%默认输出
                                ...
                                elseif varargin{2}>intmax('uint64')%参数2过大
                                    ...
                                    dynObj(uint64(varargin{1}),uint64(varargin{2}))=struct;
                                end

                            case 'cell'%此时动态内存对象被赋予为细胞数组，且申请大小不超过intmax('uint16')
                                if varargin{1}*varargin{2}>intmax('uint16')%输入参数过大
                                    dynObj=cell(intmax('uint8'),intmax('uint8'));%默认输出
                                    dynObj(:)={0};
                                    disp(['参数1：行数与参数2：列数的乘积过大，最大不能超过',...
                                        num2str(intmax('uint16')),'！']);
                                    return;
                                else%输入参数在范围内                 
                                    dynObj=cell(uint16(varargin{1}),uint16(varargin{2}));
                                    dynObj(:)={0};
                                end    
    ...
    methods(Access='public')%公开函数
		...
		%TODO:补全函数
			...
			direct=0;%方向默认为行数方向
			rowadd=dynObj.originalRow;%行数默认增加orignalRow
			coladd=0;%列数默认增加0
