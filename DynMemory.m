classdef DynMemory
    ...
    methods(Access=public)%构造函数
        function variable = DynMemory(varargin)
    ...
    methods(Access='public')%公开函数
        function refresh(DynMemoryObj)%自动刷新检测内存是否足够，有可能检测失败
            %METHOD1 此处显示有关此方法的摘要
            %   此处显示详细说明
            %如果大于额定值，就扩容
            if balabala
                addMemory(DynMemoryObj);
            end
        end       
        
        function addMemory(DynMemoryObj)%手动去申请更大的内存
            %METHOD1 此处显示有关此方法的摘要
            %   此处显示详细说明
            i%如果大于额定值，就扩容
        end             
        
        function free(DynMemoryObj)%释放内存
            %METHOD1 此处显示有关此方法的摘要
            %   此处显示详细说明
            clear DynMemoryObj;
            pack;
